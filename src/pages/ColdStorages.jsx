...
const ColdStorages = () => {
  ...
  const fetchColdStorages = async (lat, lon) => {
    ...
    try {
      const response = await axios.get(url, { timeout: 100000 }); // Timeout after 10 seconds
  ...
  const handleMoreClick = () => {
    if (location) {
      // Redirect to Google Maps to view cold storages (search query)
      const googleMapsUrl = `https://www.google.com/maps/search/cold+storage+near+me/@${location[0]},${location[1]}`;
    ...
    <div className="min-h-screen pt-20 bg-gradient-to-br from-black via-gray-900 to-black flex flex-col items-center justify-center">
      <div className="bg-black/80 backdrop-blur-lg p-8 rounded-2xl shadow-2xl border border-white/10 w-full max-w-4xl relative">
        ...
        <button
          ...
          className="absolute top-4 right-4 bg-gradient-to-r from-green-400 to-blue-500 text-white px-4 py-2 rounded-full hover:bg-blue-700"
        >
          More
        </button>
